...
- exposes an inference-side companion surface through `ProcessRewardModelAdapter` in `inference_protocols.py`, preserving the same outcome-plus-process blend while reusing the PRM's own step-boundary detection during inference-time scoring
...
This callable contract remains the training/runtime bridge inside `rlhf.py`. The companion protocol layer in `inference_protocols.py` does not replace it; instead, `ProcessRewardModelAdapter` mirrors the same outcome-plus-process blend for inference-side components that prefer `score()` and `score_steps()` surfaces.
...
The core training runtime still lives in `rlhf.py`, but the adjacent `inference_optimizations.py` file is now a much larger companion implementation for the repo's inference-time compute, search, verifier, and hidden-deliberation stack. It should be treated as a live subsystem, not as an aspirational appendix. The code on disk as of **March 25, 2026** provides real implementations for speculative decoding, best-of-N reranking, PRM-aware step scoring, paged KV cache management, MCTS-based generation, lexical-MDP search, verifier factories, two-phase hidden chain-of-thought generation, A* decoding, and search-to-training rollout collection.
...
| Total lines | 2,548 |
| Top-level classes | 20 |
| Top-level functions | 5 |
| Dominant role | Standalone inference-time compute, decoding, reranking, search, verification, and hidden-deliberation utilities |
| Main entry points | `OptimizedAttention`, `PagedKVCache`, `SpeculativeDecoder`, `BestOfNSampler`, `MCTSGenerator`, `LexicalMDP`, `VerifiableRewardFactory`, `ChainOfThoughtGenerator`, `AStarGenerator`, `TreeRolloutCollector`, `compile_model` |

### File-local banding

| Band | Primary contents |
| --- | --- |
| 1-113 | Module docstring, imports, output-adapter helpers, device resolution |
| 114-203 | `OptimizedAttention` |
| 204-510 | `PagedKVCache` plus prefix-reuse helpers and cache stats |
| 511-767 | `SpeculativeDecoderConfig`, `SpeculativeDecoder` |
| 768-1131 | `BestOfNConfig`, `BestOfNSampler` |
| 1132-1607 | `MCTSConfig`, `MCTSNode`, `MCTSGenerator` |
| 1608-1699 | `MDPConfig`, `LexicalMDP` |
| 1700-1930 | `VerifiableRewardFactory` |
| 1931-2032 | `ChainOfThoughtConfig`, `ChainOfThoughtGenerator` |
| 2033-2311 | `AStarConfig`, `AStarNode`, `AStarDecoder` |
| 2312-2490 | `RolloutSample`, `AStarGenerator`, `TreeRolloutCollector` |
| 2491-2548 | `compile_model()` and module-level usage examples |
...
flowchart TD
    ...
    B --> F[BestOfNSampler]
    B --> G[MCTSGenerator]
    B --> H[ChainOfThoughtGenerator]
    B --> I[AStarGenerator]
    G --> J[LexicalMDP]
    I --> J
    F --> K[Reward / PRM scorer]
    F --> L[Optional value model]
    G --> M[Reward fn]
    G --> N[Optional value model]
    I --> O[PRM g-score]
    I --> P[Optional value heuristic]
    M --> Q[VerifiableRewardFactory]
    H --> K
    G --> R[TreeRolloutCollector]
    I --> D
    G --> D
    C --> S[Flash Attention 2]
    C --> T[PyTorch SDPA]
```

### Output adapters, device resolution, and compatibility helpers

The first meaningful design decision in the file is still that the optimization code does not assume a single Hugging Face output type. `_extract_logits()` accepts raw tensors, dictionaries with `logits` or `scores`, and object-like outputs exposing `logits` or `scores` attributes. `_extract_past_key_values()` does the same for cache-aware generation. `_extract_scalar_output()` is even more permissive: it reduces tensors directly, looks for `score`, `reward`, `rewards`, `value`, `values`, or `logits` in dict or object outputs, and returns `0.0` if no recognizable scalar-like signal exists. `_resolve_model_device()` completes that compatibility layer by resolving device placement from wrapped or raw modules without assuming a `.device` attribute.

That helper layer matters because the module now relies heavily on duck-typed policy, reward, value, PRM, and verifier objects. In practice, this file is written to sit on top of heterogeneous wrappers and adapters rather than a single canonical model API, which is why `inference_protocols.py` exists as a direct companion rather than an optional convenience.
...
The new implementation detail that matters most is prefix reuse. `get_or_compute_prefix()` tokenizes a shared prompt prefix once, runs the policy model with `use_cache=True`, and stores both `past_key_values` and the last-step logits so later search nodes can replay only their suffix tokens. `MCTSGenerator._rollout()` and `AStarDecoder._cached_legal_actions()` both use that prefix entry when they can. This is what turns the cache from a passive storage class into an active search accelerator.

The important implementation truth is still that this interface is more mature than the underlying storage model. `num_layers` is accepted and stored, and both `append_token()` and `get_kv()` accept `layer_idx`, but the actual cache tensor has no layer dimension and neither method indexes by layer. As written today, the class behaves like a single-layer KV store with a multi-layer-looking API.
...
The file has also grown a PRM-aware path that matters for reasoning tasks. `score_step_by_step()` implements a weakest-link "PRM-Min" aggregation rule: if the supplied scorer exposes `score_steps()`, the sampler uses that fast path and returns the minimum per-step score; otherwise it falls back to delimiter-based splitting plus per-step scoring. That makes `BestOfNSampler` the first place where `ProcessRewardModelAdapter` can directly change reranking behavior instead of just final scalar scoring.
...
Two integration details matter in the current file. First, the generator optionally accepts `mdp`, which lets it operate over `LexicalMDP` transitions rather than only raw string concatenation. Second, `_rollout()` can reuse `PagedKVCache.get_or_compute_prefix()` to avoid recomputing the shared prompt prefix on every simulation. So the current implementation is no longer just "MCTS plus optional value model"; it is MCTS plus a lightweight search-state abstraction and an explicit prefix-reuse optimization path.

There are also several very specific implementation boundaries that matter when reading or extending this class. Node state is stored as decoded text, not token IDs. `_is_terminal()` checks string length (`len(state) > 2000`) and string suffix against `tokenizer.eos_token`, while `_rollout()` computes `max_new_tokens` from tokenized input length. So the class still mixes token-space and string-space stopping rules in the current implementation.

### `LexicalMDP`

`LexicalMDP` is the file's main formal bridge from text generation to search. It defines:

- `initial_state(prompt)`
- `transition(state, action)`
- `is_terminal(state)`
- `legal_actions(state, n, temperature)`
- `reward(state, reward_fn)`

This matters because both `MCTSGenerator` and `AStarDecoder` can operate as clients of the same state-transition abstraction. States are still strings and actions are still decoded token strings, but the module now has an explicit place to talk about generation as an MDP rather than only as ad hoc autoregressive sampling.

### `VerifiableRewardFactory`

`VerifiableRewardFactory` adds a second major expansion to the module: deterministic verifier construction. The current factory methods include:

- `math_verifier()` for answer extraction plus exact/normalized comparison
- `code_verifier()` for sandboxed pass-rate style test-case execution
- `format_verifier()` for structural constraints such as required tags, forbidden regexes, and minimum step counts

All returned callables map completions to floats in `[-1.0, 1.0]`. In practice, these functions are the inference-time side of verifier-guided search: they are the easiest reward surfaces to pass into `MCTSGenerator.generate()` or `AStarGenerator.generate()` when the caller wants deterministic external checks instead of, or in addition to, learned reward/value models.

### `ChainOfThoughtGenerator`

`ChainOfThoughtGenerator` is the module's explicit two-phase hidden-deliberation primitive. `ChainOfThoughtConfig` defines:
...
Runtime behavior is simple but important:

1. Build a phase-1 prompt ending in `think_start_tag`.
2. Generate reasoning until `think_end_tag`.
3. Build a full context containing the hidden reasoning block.
4. Generate the answer text in a second phase.
5. Return both the full hidden sequence and the answer-only sequence.

If `prm_scorer` is supplied, the generator also returns `thinking_score`, which lets a PRM evaluate the hidden reasoning text directly. That makes this class a concrete implementation site for hidden-deliberation serving rather than a placeholder doc idea.

### `AStarDecoder` and `AStarGenerator`

The file now contains a full A* search lane. `AStarDecoder` runs best-first search over `LexicalMDP` states with:

- `g_score` from cumulative PRM score along the path
- `h_score` from an optional value model heuristic
- `max_nodes`, `max_depth`, and `n_actions` controlling frontier growth
- optional `PagedKVCache` prefix reuse during node expansion

`AStarGenerator` is the convenience wrapper that mirrors `MCTSGenerator.generate(prompt, reward_fn) -> dict` while wiring together `LexicalMDP`, `AStarDecoder`, and optional PRM/value inputs.

This is also where the process-reward story becomes most explicit. In the current implementation, A* is the most PRM-native search path in the module: the PRM can supply both path accumulation (`g_score`) and, when paired with a value head, part of the search heuristic story.

### `TreeRolloutCollector`

`TreeRolloutCollector` is the search-to-training bridge inside the file. It runs MCTS over a list of prompts, extracts the top leaf paths by visit count, filters them by reward threshold, and returns `RolloutSample` objects with:
...
The important boundary is that the collector stops at sample construction. It does not call `GRPOTrainer` or `RLHFOrchestrator` directly. So the bridge to training data is real, but it is still a companion-layer bridge rather than an orchestrator-integrated stage in `rlhf.py`.
...
This companion module is real code, but it is not currently the same kind of runtime center that `rlhf.py` is. A repo-wide search shows the main references in `benchmark_harness.py`, `inference_protocols.py`, and documentation files. It is not wired into `RLHFOrchestrator` as a first-class stage. So the correct mental model is:

- `rlhf.py` owns the end-to-end training and persistence pipeline.
- `inference_optimizations.py` owns a standalone inference toolkit that can be instantiated by benchmark, protocol, or manual experiment layers.
- `inference_protocols.py` is the typed bridge that makes `rlhf.py` models usable inside this toolkit without ad hoc attribute assumptions.
...
| Search-state representation | both MCTS and A* still store node state as decoded strings rather than token-ID graphs |
| PRM-aware reranking | `score_step_by_step()` can exploit `score_steps()`, but only when the caller supplies a compatible adapter or scorer |
| Orchestrator integration | the module is used by inference-side code, not integrated into `RLHFOrchestrator` |

---

## Companion Module: `inference_protocols.py`

### Why it belongs in this internal spec

`inference_protocols.py` is the contract layer that makes the expanded inference stack usable with the training/runtime objects defined in `rlhf.py`. It is not just type decoration. The file defines the protocol surfaces expected by `inference_optimizations.py` and wraps `PolicyModel`, `RewardModel`, `ValueModel`, hidden-deliberation generators, verifiers, and `ProcessRewardModel` objects so search and reranking code does not depend on ad hoc `.device`, `.score_text`, or output-layout assumptions.

### Snapshot

| Item | Value |
| --- | --- |
| Primary source | `inference_protocols.py` |
| Total lines | 843 |
| Top-level classes | 15 |
| Top-level functions | 0 |
| Dominant role | Typed contracts and adapter implementations connecting `rlhf.py` model surfaces to `inference_optimizations.py` |
| Main entry points | `PolicyAdapter`, `RewardScorerAdapter`, `ValueScorerAdapter`, `HiddenCoTAdapter`, `VerifiableRewardAdapter`, `StepLevelScorerAdapter`, `ProcessRewardModelAdapter` |

### File-local banding

| Band | Primary contents |
| --- | --- |
| 1-116 | Module docstring, imports, logging, protocol errors, protocol definitions |
| 117-191 | `PolicyAdapter` |
| 192-255 | `RewardScorerAdapter` |
| 256-313 | `ValueScorerAdapter` |
| 314-494 | `HiddenCoTAdapter` |
| 495-537 | `VerifiableRewardAdapter` |
| 538-615 | `StepLevelScorerAdapter` |
| 616-843 | `ProcessRewardModelAdapter` |

### System graph

```mermaid
flowchart LR
    A[rlhf.PolicyModel] --> B[PolicyAdapter]
    C[rlhf.RewardModel] --> D[RewardScorerAdapter]
    E[rlhf.ValueModel] --> F[ValueScorerAdapter]
    G[rlhf.ProcessRewardModel] --> H[ProcessRewardModelAdapter]
    I[Generator with hidden reasoning] --> J[HiddenCoTAdapter]
    K[Rule-based verifier or callable] --> L[VerifiableRewardAdapter]
    M[Step scorer or callable] --> N[StepLevelScorerAdapter]
    B --> O[PolicyLike]
    D --> P[RewardScorerLike]
    F --> Q[ValueScorerLike]
    H --> P
    H --> R[StepLevelScorerLike]
    J --> S[HiddenCoTLike]
    L --> T[VerifiableRewardLike]
    N --> R
    O --> U[inference_optimizations.py]
    P --> U
    Q --> U
    R --> U
    S --> U
...
The file defines six runtime-checkable contracts:

- `PolicyLike`
- `RewardScorerLike`
- `ValueScorerLike`
- `HiddenCoTLike`
- `VerifiableRewardLike`
- `StepLevelScorerLike`

Two design choices matter here. First, the protocols are intentionally small. They describe only the methods the inference components actually need. Second, the file includes explicit `InferenceProtocolError` and `ProtocolValidationError` classes, so adapter failures are surfaced as contract violations rather than obscure attribute errors deep in search code.

### `PolicyAdapter`, `RewardScorerAdapter`, and `ValueScorerAdapter`

These three adapters are the base bridge from `rlhf.py` into the inference stack.

- `PolicyAdapter` unwraps `rlhf.PolicyModel` when needed, normalizes dict outputs into attribute-style objects, exposes `generate()`, and resolves device placement via parameter inspection.
- `RewardScorerAdapter` tokenizes prompt/completion text, runs the wrapped reward model, and exposes both `score()` and `score_batch()`.
- `ValueScorerAdapter` tokenizes text, prefers `return_all_values=False` when supported, and exposes `score()` / `score_text()` for heuristic-style consumers.

The practical effect is that `BestOfNSampler`, `MCTSGenerator`, and `AStarGenerator` can consume a stable scoring surface even when the wrapped objects are raw HF modules, `rlhf.py` wrapper classes, or other compatible modules.

### `HiddenCoTAdapter`

`HiddenCoTAdapter` is the strict contract enforcer for hidden-deliberation serving. It validates:

- non-empty prompt and token budgets
- distinct thinking start/end tags
- presence and ordering of those tags in the full sequence
- answer-only output after sanitization

It prefers a native `generate_with_hidden_cot()` path when available, but can fall back to a generic `generate()` path and normalize either tuple or dict-style results. It also strips hidden-thinking tags from the answer-only output if the upstream generator leaks them. So this adapter does more than type adaptation; it enforces the "hidden reasoning in, answer-only out" serving contract.

### `VerifiableRewardAdapter`

`VerifiableRewardAdapter` normalizes deterministic verifiers to `verify(completion, **kwargs) -> float`. It accepts objects with `verify()`, objects with `score()`, or plain callables. It also clamps scores to `[-1.0, 1.0]` by default and rejects non-finite outputs. This is the protocol-side counterpart to `VerifiableRewardFactory` in `inference_optimizations.py`.

### `StepLevelScorerAdapter` and `ProcessRewardModelAdapter`

`StepLevelScorerAdapter` is the generic step-scoring adapter. It can:

- consume native `score_steps()`
- split a completion by a delimiter and score each segment
- clamp out-of-range values into a configured numeric band
- inject a neutral score when the upstream scorer returns an empty list

`ProcessRewardModelAdapter` is the more important inference companion because it wraps the real `ProcessRewardModel` from `rlhf.py`. It exposes:

- `score_outcome()` for final outcome reward
- `score_steps()` for per-step process rewards
- `score_combined()` for the blended outcome-plus-process contract
- `score()` as the primary scalar entry point
- `score_batch()` with batched tokenization plus sequential fallback

The implementation detail that matters most is boundary detection. `score_steps()` uses the PRM's own token-aligned process-reward extraction instead of naive delimiter splitting. That is the inference-side correctness improvement over `StepLevelScorerAdapter`.

The second critical detail is contract alignment with `rlhf.py`. `score_combined()` mirrors `RewardFunctionFactory.from_process_reward_model()`:

```python
reward = outcome_reward + process_weight * mean(process_rewards[:num_steps])
```

So the same PRM can participate in both training-side reward construction and inference-side reranking/search without silently changing how the scalar reward is formed.

### Wiring reality in the repo

This file is the narrowest bridge between the training runtime and the inference toolkit:

- `rlhf.py` owns model construction, training, persistence, and orchestrated policy optimization.
- `inference_protocols.py` adapts those trained objects into stable protocol surfaces.
- `inference_optimizations.py` consumes those protocol surfaces for reranking, search, hidden-deliberation serving, and verifier-guided generation.

That division is important because it explains why the repo can stay RLHF-first while still exposing a much richer inference/search lane.

### Companion-module caveats that should be treated as implementation facts

| Topic | Current reality in `inference_protocols.py` |
| --- | --- |
| Policy wrapping | `PolicyAdapter` normalizes outputs and device access, but still delegates generation semantics to the wrapped model |
| Hidden-CoT enforcement | `HiddenCoTAdapter` validates tags and sanitizes answer-only output, but cannot invent hidden reasoning if the underlying generator never emits it |
| Step scoring fallback | `StepLevelScorerAdapter` falls back to delimiter splitting when no native `score_steps()` exists |
| PRM batch scoring | `ProcessRewardModelAdapter.score_batch()` tries a true batch forward pass first, then falls back to sequential scoring on any tokenizer/model incompatibility |
| Contract alignment | `ProcessRewardModelAdapter` mirrors `RewardFunctionFactory.from_process_reward_model()` rather than replacing the callable reward-factory path inside `rlhf.py` |
